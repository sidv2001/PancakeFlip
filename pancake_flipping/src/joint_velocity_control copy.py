...
import logging
import numpy as np
from ar_track_alvar.ar_track_alvar_msgs import SyncedPose
...
def pose_cb(synced_pose : SyncedPose):
    p = synced_pose.avg_marker.pose.pose
    q = synced_pose.avg_marker.pose.orientation
    state_vec = np.zeros(10)
    state_vec[0:7] = [p.x, p.y, p.z, q.x, q.y, q.z, q.w]
    state_vec[7] = synced_pose.cam
    state_vec = synced_pose.avg_marker
...
def main():
    ...
    synced_pose_sub = rospy.Subscriber("chatter", SyncedPose, pose_cb)
