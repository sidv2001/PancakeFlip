...
def get_joint_velocity(row):
    robot_joint_angles = row.split(", ")
    robot_joint_angles[0] = robot_joint_angles[0][1:]
    robot_joint_angles[-1] = robot_joint_angles[-1][:-1]
    fin = np.array(list(map(lambda x: float(x), robot_joint_angles)))
    norm = np.linalg.norm(fin, ord=1)
    return norm
...
def get_dataset(dataset_name):
    ...
    for file in files:
        if file[-3:] == "csv":
            ...
            zerovel = True
            length = len(df["joint_states.velocity"])
            j = 0
            while zerovel != False:
                norm = get_joint_velocity(df["joint_states.velocity"][j])
                if norm <= 0.01:
                    # print(robot_joint_angles)
                    df = df.drop(j)
                    j += 1
                else: 
                    zerovel = False
            ...
            i = end
            while zerovel != False:
                norm = get_joint_velocity(df["joint_states.velocity"][i])
                next_norm = get_joint_velocity(df["joint_states.velocity"][i-1])
                if norm <= 0.01 and next_norm <= 0.01:
                    # print(robot_joint_angles)
                    df = df.drop(i)
                    i -= 1
                else: 
...
def get_dataset_vel(dataset_name):
    ...
    for file in files:
        if file[-3:] == "csv":
            df = pd.read_csv(os.path.join(dataset_dir, file))
            zerovel = True
            print(len(df["joint_states.velocity"]))
            length = len(df["joint_states.velocity"])
            j = 0
            while zerovel != False:
                norm = get_joint_velocity(df["joint_states.velocity"][j])
            ...
            while zerovel != False:
                ...
                if norm <= 0.01 and next_norm <= 0.01:
                    ...
                    zerovel = False
            print("Datset Name: ", file)
            print("Deleted ", j, " examples from the front")
            print("Deleted ", end - i, " examples from the back")
            X, Y = create_tensors_vel(df)
            X = X[:-1]   
            Y = Y[1:]  
            finX.append(X)
            finY.append(Y)
...
def create_tensors_vel(df):
    finX = []
    finY = []
    for index, row in df.iterrows():
        X,Y = convert_pandas_row_to_tensor_vel(row)
        x = X.shape[0]
        y = Y.shape[0]
        X = X.reshape(1, x)
        Y = Y.reshape(1, y)
...
def createX(px, py, pz, ox, oy, oz, ow, c1, c2, c3, r1, r2, r3, r4, r5, r6):
    ...
    """
    return torch.tensor([px,py,pz,ox,oy,oz,ow,c1,c2,c3,r1,r2,r3,r4,r5,r6])
    # return torch.tensor([px,py,pz,ox,oy,oz,ow,c1,c2,c3,r1,r2,r3,r4,r5,r6, v1, v2, v3, v4, v5 ,v6])

def create_X_vel(px, py, pz, ox, oy, oz, ow, c1, c2, c3, r1, r2, r3, r4, r5, r6, v1, v2, v3, v4, v5 ,v6):
    """Converts the following arguments into a tensor
...
def convert_pandas_row_to_tensor(row):
    ...
    X = createX(px, py, pz, ox, oy, oz, ow, c1, c2, c3, r1, r2, r3, r4, r5, r6)
    Y = createY(v1, v2, v3, v4, v5, v6)
    return X, Y

def convert_pandas_row_to_tensor_vel(row):
    """_summary_
    ...
    X = create_X_vel(px, py, pz, ox, oy, oz, ow, c1, c2, c3, r1, r2, r3, r4, r5, r6, v1, v2, v3, v4, v5, v6)
    Y = createY(v1, v2, v3, v4, v5, v6)
    return X, Y
...
def convert_msg_to_tensor_vel(msg, scalar=1):
    """_summary_
    ...
    Returns:
        ...
        Y: Y Tensor
    """
    X = torch.tensor([msg.avg_marker.pose.pose.position.x,
                      msg.avg_marker.pose.pose.position.y,
                      msg.avg_marker.pose.pose.position.z,
                      msg.avg_marker.pose.pose.orientation.x,
                      msg.avg_marker.pose.pose.orientation.y,
                      msg.avg_marker.pose.pose.orientation.z,
                      msg.avg_marker.pose.pose.orientation.w,
                      (1 if msg.cam1_marker.header.frame_id == "wx250s/base_link" else -1),
                      (1 if msg.cam2_marker.header.frame_id == "wx250s/base_link" else -1),
                      (1 if msg.cam3_marker.header.frame_id == "wx250s/base_link" else -1),
                      msg.joint_states.position[0],
                      msg.joint_states.position[1],
                      msg.joint_states.position[2],
                      msg.joint_states.position[3],
                      msg.joint_states.position[4],
                      msg.joint_states.position[5],
                      msg.joint_states.velocity[0]*scalar,
                      msg.joint_states.velocity[1]*scalar,
                      msg.joint_states.velocity[2]*scalar,
                      msg.joint_states.velocity[3]*scalar,
                      msg.joint_states.velocity[4]*scalar,
                      msg.joint_states.velocity[5]*scalar ])
    return X


if __name__ == "__main__":
    print(get_dataset("new_data"))
