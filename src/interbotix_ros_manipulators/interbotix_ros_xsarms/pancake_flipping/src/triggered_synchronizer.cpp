...
interbotix_xs_msgs::JointGroupCommand start_msg, halt_msg;
...
int main(int argc, char **argv) {
  ...
  std::vector<float> start_cmd = {0, 0.4, -0.2, 0, -0.25, 0};
  std::vector<float> halt_cmd = {0, 0, 0, 0, 0, 0};
  start_msg.name = "arm";
  start_msg.cmd = start_cmd;
  halt_msg.name = "arm";
  halt_msg.cmd = halt_cmd;

  ros::Time beginTime = ros::Time::now();
  while(ros::Time::now() < beginTime + ros::Duration(6) ) {
    pub_group.publish(start_msg);
    ros::Duration(0.01).sleep();
  }
  pub_group.publish(halt_msg);
  ros::topic::waitForMessage<std_msgs::String>("first_spacebar");
  system("rosservice call /wx250s/torque_enable group arm false");
  ...
  ros::topic::waitForMessage<std_msgs::String>("second_spacebar");
